<template>
    <div ref="easterModal" class="modal inmodal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content animated flipInY">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span
                                class="sr-only">Close</span></button>
                    <h4 class="modal-title"><strong>Enduro<span class="text-warning">+</span></strong></h4>
                </div>
...
export default {
    mounted() {
        ...
        Event.$on('showEaster', () => {
            ...
            $(this.$refs.easterModal).modal('show');
        ...
        cars.create = function (i,j) {  
            ...
            c.className = 'car';
            c.setAttribute(_this.$options._scopeId, '');
            cars.color(c);
            var d = road.width * 0.42, 
                w = road.width - d - car.width; 
            c.x = (d/2) + (i * (w/2)); 
